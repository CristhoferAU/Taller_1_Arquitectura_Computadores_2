...
// Histograma paralelo con histograma global protegido por sección crítica
std::vector<size_t> histograma_global_critical(const std::vector<int>& datos, int min_val, int max_val) {
...
int main() {
    ...
    std::vector<size_t> hist = histograma_global_critical(datos, min_val, max_val);
    ...
    std::cout << "Histograma GLOBAL+CRITICAL en " << tiempo_hist.count() << " segundos\n";
